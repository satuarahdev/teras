...
async function main() {
  console.log('=== Teras (API Gateway) Git Commit & Push ===\n');
  const dir = __dirname;
  
  console.log('Checking Teras...');
  const status = runCmd('git status --porcelain', dir);
  
  if (status) {
    console.log('Changes detected in Teras!');
    ...
    let msg = process.argv[2] || 'chore: automated deployment script update';
